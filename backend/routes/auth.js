...
router.post('/register', async (req, res) => {
  try {
    ...
    logger.info('User registration attempt', { email: normalizedEmail, name: normalizedName });

    // Input validation
    if (!normalizedEmail || !normalizedName || !password) {
      logger.warn('Registration failed: Missing required fields', { email: normalizedEmail, name: normalizedName });
    ...
    if (!passwordValidation.valid) {
      logger.warn('Registration failed: Weak password', { email: normalizedEmail });
    ...
    if (existingEmail) {
      logger.warn('Registration failed: Email already registered', { email: normalizedEmail });
    ...
    if (!allowedRoles.includes(validRole)) {
      logger.warn('Registration failed: Invalid role', { email: normalizedEmail, role: validRole });
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    if (!user.isActive) {
      logger.warn('Token refresh failed: Account not active', { userId: user._id });
      return res.status(401).json({ message: 'Account not active' });
    }

    if (user.disabled) {
      logger.warn('Token refresh failed: Account disabled', { userId: user._id });
...
router.post('/request-reset-otp', async (req, res) => {
  try {
    ...
    if (!checkOtpRateLimit(normalizedEmail)) {
      logger.warn('OTP request blocked: Rate limit exceeded', { email: normalizedEmail });
      return res.status(429).json({ message: 'Too many OTP requests. Please try again later.' });
    }

    const user = await User.findOne({ email: normalizedEmail });
    if (!user) {
      logger.info('OTP request for non-existent email', { email: normalizedEmail });
    ...
    logger.info('OTP generated and saved', { userId: user._id, email: user.email });
...
router.post('/verify-reset-otp', async (req, res) => {
  try {
    ...
    if (!user || !user.resetOtpCode || !user.resetOtpExpires) {
      logger.warn('OTP verification failed: No OTP request found', { email: normalizedEmail });
      return res.status(400).json({ message: 'Invalid request. Please request an OTP first.' });
    }

    if (user.resetOtpCode !== otp) {
      logger.warn('OTP verification failed: Invalid OTP', { email: normalizedEmail });
      return res.status(400).json({ message: 'Invalid OTP' });
    }

    if (Date.now() > new Date(user.resetOtpExpires).getTime()) {
      logger.warn('OTP verification failed: OTP expired', { email: normalizedEmail });
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (!normalizedEmail || !otp || !newPassword) {
      logger.warn('Password reset failed: Missing required fields', { email: normalizedEmail });
    ...
    if (!passwordValidation.valid) {
      logger.warn('Password reset failed: Weak password', { email: normalizedEmail });
      return res.status(400).json({ message: passwordValidation.message });
    }

    const user = await User.findOne({ email: normalizedEmail });
    if (!user || !user.resetOtpCode || !user.resetOtpExpires) {
      logger.warn('Password reset failed: No OTP request found', { email: normalizedEmail });
      return res.status(400).json({ message: 'Invalid request. Please request an OTP first.' });
    }

    if (user.resetOtpCode !== otp) {
      logger.warn('Password reset failed: Invalid OTP', { email: normalizedEmail });
      return res.status(400).json({ message: 'Invalid OTP' });
    }

    if (Date.now() > new Date(user.resetOtpExpires).getTime()) {
      logger.warn('Password reset failed: OTP expired', { email: normalizedEmail });
      return res.status(400).json({ message: 'OTP has expired. Please request a new one.' });
    }
